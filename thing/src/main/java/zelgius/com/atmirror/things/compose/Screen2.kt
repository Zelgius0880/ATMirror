...
data class Screen2(val forecast: List<ForecastData>) {
    var bitmap: Bitmap? = null


    constructor(forecast: List<ForecastData>, bitmap: Bitmap) : this(forecast) {
        this.bitmap = bitmap
    }
}
...
@Composable
fun Screen2View(forecast: List<ForecastData>) {
    Column(Modifier.size(300.dp, 400.dp)) {
        if (forecast.size >= 6) {
            (0 until 6 step 2).forEach {
                Row {
                    ForecastCell(item = forecast[it])
                    ForecastCell(item = forecast[it + 1])
                }
            }
        }
    }
...
private fun Preview() {
    ...
    Screen2View(Gson().fromJson(sample, OpenWeatherMap::class.java).list)
    //}
}
