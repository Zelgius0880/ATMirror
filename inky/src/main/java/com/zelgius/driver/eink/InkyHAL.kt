...
val TAG = InkyHAL::class.simpleName
...
abstract class InkyHAL(
    ...
    val buffer = Array(resolution.second) { IntArray(resolution.first) { 0 } }
    ...
    open suspend fun show(wait: Boolean = true) {
        val region = buffer
            .let { b ->
        ...
        update(
            ...
            bufferB = ArrayUtils.packToBits(
                array = region
                    .map {
                        it.map { code -> code == 2 /* Color code of RED or YELLOW */ }
                            .toBooleanArray()
                    }
                    .toTypedArray()
            ),
    ...
    suspend fun clean(cycles: Int = 3) {
        for (i in 0 until cycles) {
            println("Cleaning cycle ${i + 1}")
            listOf(color, InkyColor.BLACK, InkyColor    .WHITE).distinct().forEach {c->
                println("Updating $c")
                borderColor = c
                buffer.forEach {
                    it.fill(c.code)
